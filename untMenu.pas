...
if ShortDateFormat <> 'dd/MM/yyyy' then
  begin
    MessageDlg('Favor arrumar data para dd/mm/aaaa'+#13+#10+#13+#10+
                'Painel de Controle > Opções regionais e de idioma > '+#13+#10 +
...
if (key = vk_f9)  THEN
 ...
 if st.Panels[0].Text = 'Usuário: ADMINISTRADOR' then
  begin
...
repeat
Resposta := InputBox('Salvar arquivos excel em:', 'Digite a pasta (Não esqueça de colocar ''\'' no final do endereço):',atual);
until Resposta <> '';
ArqIni := tIniFile.Create(extractfilepath(application.exename)+'\config');
...
begin
   If Pos(UpperCase('is not a valid date'), UpperCase(E.Message)) <> 0 then
      ...
      If Pos(UpperCase('key violation'), UpperCase(E.Message)) <> 0 then
           MESSAGEDLG('Registro já incluido!',MTERROR,[MBOK],0)
   else
...
frmRelatGrafico4.vMenu := 'VALOR MONETÁRIO';
